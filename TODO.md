...
>
> **Decomposition progress** — most of 18.8's surface already shipped under Phase 21: Live Positions (21.3), Force-close/Modify (21.4), Exposure-by-symbol (21.5), KYC list (existing admin/kyc; inline image preview still tracked by 21.7). Remaining unbuilt sub-pages are carved out below as 18.8x items so an auto-run can take one at a time.

### 18.8a Robot run log — `GET /api/admin/robot-runs`
- [x] **Done 2026-06-22 (auto):** Backend `GET /api/admin/robot-runs` (`server/src/routes/admin.ts`, admin-only via `authAdmin`) — paginated, filterable operator log of `robot_runs` (the manager panel's "Robot Runs" page). Since `robot_runs -> robots(account_id) -> accounts(login)` is a two-hop relation with no direct FK to PostgREST-embed, the route stitches in-route: it builds a `robot_id -> {name, account_id}` map and an `account_id -> {login, user_id}` map, then joins each run. Query params (all optional): `from`/`to` (ISO bounds on `triggered_at`, gte/lte), `action` (exact: open_trade|close_trade|tip|noop), `robot` (exact robot_id), `account` (login NUMBER resolved to account_id, or raw id; unknown login -> empty set, not an error), `dir` (asc|desc on triggered_at, default desc), `limit` (1..500 default 100), `offset` (>=0 default 0). Each row: id, robot_id, robot_name, account_id, login, user_id, triggered_at, action, trade_id, notes. `totals` (count, trades_opened, per-action `by_action` breakdown) computed over the FULL filtered set so it reconciles against raw `robot_runs`; `runs` is the sorted page slice. `lib/api.ts`: `api.adminGetRobotRuns(params)` typed helper. Test helper extended (additive): `seed.robot()` + `seed.robotRun()` in `supabaseMock.ts`. New `server/test/adminRobotRuns.test.ts` (10 tests: 403 unauth/non-admin; full listing w/ robot-name+login stitch and reconciling totals [count 4, trades_opened 2, by_action {open_trade:2,noop:1,tip:1}]; filter by action / robot / account-login; unknown-login -> empty; triggered_at date-range; limit/offset paging with full-set totals + no overlap; dir=asc flip). No migration (robot_runs exists since 001_init). Verified offline: client tsc clean, server tsc clean, `npm test` **265 passing** (was 255). PENDING LIVE VERIFY (next interactive session): trigger a robot run on the live DB -> it appears in `/api/admin/robot-runs` with correct stitch; the admin "Robot Runs" screen (UI — separate sub-item 18.8b) consumes this endpoint.
- **Files:** `server/src/routes/admin.ts`, `lib/api.ts`, `server/test/adminRobotRuns.test.ts`, `server/test/helpers/supabaseMock.ts`
- **Acceptance:** Endpoint returns robot_runs with robot name + owner login; filtering by date/user/result narrows the set; totals reconcile against raw robot_runs. *(Offline unit-testable — met.)*
### 18.8e/18.8f Deposits & Withdrawals admin slices — `type` filter on `GET /api/admin/transactions`
- [x] **Done 2026-06-22 (auto):** The manager-panel Deposits and Withdrawals pages each need to list just their own slice of `transactions`. Approve/reject already exist generically since 4.3 (`POST /api/admin/transactions/:id/approve|reject` branch on `tx.type` — deposit credits, withdrawal debits with a sufficient-funds check), so the only missing backend piece was the read slice: the list route filtered by `status` only. Added an optional `type` query param to `GET /api/admin/transactions` (`server/src/routes/admin.ts`), whitelisted to `deposit|withdrawal|bonus|adjustment` (unknown/garbage type ignored → full set, never an error), composable with the existing `status` filter (e.g. `?status=pending&type=withdrawal` → pending withdrawals only). `lib/api.ts`: `api.adminGetTransactions(status, type?)` — second arg optional, backward-compatible with all existing call sites (built via `URLSearchParams`). Test helper: added a transactions account-embed branch to `supabaseMock.ts` (additive) so the Withdrawals page's "balance next to the request" is asserted. New `server/test/adminTransactions.test.ts` (8 tests: 403 unauth/non-admin; full set w/ embedded account balance+user_id; status=pending slice; type=deposit slice; type=withdrawal slice; status+type compose → pending withdrawals; garbage type ignored). No migration (transactions table + approve/reject pre-exist). Verified offline: client tsc clean, server tsc clean, `npm test` **273 passing** (was 265). This satisfies BOTH 18.8e and 18.8f at the backend level — the two pages differ only by `type=deposit` vs `type=withdrawal`. PENDING LIVE VERIFY (next interactive session): hit `/api/admin/transactions?type=withdrawal` on the live DB and approve one. The admin Deposits/Withdrawals **screens** (UI) remain a separate visual sub-item.
- **Files:** `server/src/routes/admin.ts`, `lib/api.ts`, `server/test/adminTransactions.test.ts`, `server/test/helpers/supabaseMock.ts`
- **Acceptance:** Admin can request a single transaction-type slice (deposits-only / withdrawals-only), optionally combined with a status filter, with the owning account (balance) embedded; approve/reject of either type already works. *(Offline unit-testable — met.)*
### 18.8c Price-alerts log — `GET /api/admin/alerts`
- [x] **Done 2026-06-22 (auto):** Backend `GET /api/admin/alerts` (`server/src/routes/admin.ts`, admin-only via `authAdmin`) — paginated, filterable operator log over the `price_alerts` table (exists since `008_price_alerts.sql`; no migration). `price_alerts.user_id` IS the auth user id, so each row is stitched to the owning user's account login (first account per user, via accounts.user_id) and profile display_name (profiles.id). An alert's `status` is derived: `active` while `triggered_at` is null, else `triggered`. Query params (all optional): `status` (active|triggered, else both — applied in-route since it's a triggered_at predicate the PostgREST mock doesn't `.is()`-filter), `symbol` (exact), `direction` (above|below), `account` (login NUMBER resolved to its user_id; unknown login → empty set, not an error), `user` (raw auth uid, takes precedence over account), `from`/`to` (ISO bounds on created_at, gte/lte), `dir` (asc|desc on created_at, default desc), `limit` (1..500 default 100), `offset` (>=0 default 0). Each row: id, user_id, login, display_name, symbol, threshold (numeric), direction, status, triggered_at, created_at. `totals` (count, active, triggered, per-direction `by_direction`) computed over the FULL filtered set so it reconciles against raw `price_alerts`; `alerts` is the sorted page slice. `lib/api.ts`: `api.adminGetAlerts(params)` typed helper. Test helper extended (additive): `price_alerts` table + `seed.priceAlert()` in `supabaseMock.ts`. New `server/test/adminAlerts.test.ts` (12 tests: 403 unauth/non-admin; full listing w/ login+display_name stitch and reconciling totals [count 4, active 2, triggered 2, by_direction {above:2,below:2}]; status=active/triggered; symbol; direction; account-login; unknown-login → empty; created_at date-range; limit/offset paging with full-set totals + no overlap; dir=asc flip). Verified offline: client tsc clean, server tsc clean, `npm test` **285 passing** (was 273). PENDING LIVE VERIFY (next interactive session): create a price alert on live → it appears in `/api/admin/alerts` with correct login/display_name stitch; let it fire → status flips to `triggered`. The admin "Price Alerts" **screen** (UI) remains a separate visual sub-item.
- **Files:** `server/src/routes/admin.ts`, `lib/api.ts`, `server/test/adminAlerts.test.ts`, `server/test/helpers/supabaseMock.ts`
- **Acceptance:** Endpoint returns every price alert with owner login + display_name and an active/triggered status; filtering by status/symbol/direction/account/date narrows the set; totals reconcile against raw price_alerts. *(Offline unit-testable — met.)*
> **Remaining 18.8 sub-items (not yet carved/done):** 18.8b admin "Robot Runs" screen UI (visual); 18.8c **screen** (the Price Alerts page UI consuming the route above — visual); 18.8d AI chat-logs log (depends on 18.7 assistant). The Deposits/Withdrawals admin **screen UIs** (consuming the 18.8e/18.8f slices above) are visual sub-items. Each should be its own bounded item.
...
> STATIC AUDIT DONE 2026-06-26 (auto): `docs/admin-audit.md` now exists — every one of the 24 `/api/admin/*` routes was cross-checked, column-by-column, against the migration schema (001–031). Result: **no column-name/embed bug** (the historical `opened_at`→`open_time` and profiles↔accounts-embed defects are confirmed fixed and not regressed). One non-code caveat: `/online` needs migration 031 (`accounts.last_seen`) applied to live or it 500s. Client+server tsc clean. Box stays `[ ]` because acceptance still requires a LIVE 200 from each route (network-gated) — only the offline half is done; resume the live-curl pass per the doc's 'Live verification still required' section.
...
- [x] **Files:** `app/admin/users.tsx`, possibly `server/src/routes/admin.ts` (`/users`)
...
> 2026-07-09 (auto run 74): now scoped by `docs/account-groups-design.md` §3.3 — this is satisfied by **21.14e** (group-configurable stop-out in `risk.ts`). Recommendation: ship the group-level version first; add an `accounts.stopout_level` per-account override only if the user explicitly needs it. Dependency narrows from all of 21.14 to just 21.14b (schema) + 21.14e.
...
> DECOMPOSED 2026-07-09 (auto run 74): the "design and scope as its own mini-phase" step the item mandates is now done — see `docs/account-groups-design.md` (grounded in the real `accounts` schema, `risk.ts` stop-out rule, and `pricefeed.ts` spread). It carves 21.14 into offline-completable slices 21.14a–f below and defines how per-group markup/leverage/stop-out fold into the existing pricing/risk layers. 21.12 (per-account stop-out) folds into 21.14e. Parent stays `[ ]` until 21.14b–e ship + apply + live-verify. Future runs: pick 21.14b next (migration 032, offline).
...
### 21.14a Design & scope mini-phase — `docs/account-groups-design.md`
- [x] **Done 2026-07-09 (auto run 74):** Wrote `docs/account-groups-design.md` — the design/scope pass 21.14 requires as its first step (parallels 21.8's `docs/mt4-manager-parity.md` and 21.1's `docs/admin-audit.md`). Grounded in the current codebase: `accounts` has `leverage`/`equity`/`margin_used`/`free_margin` but no `group_id`/`credit`/`stopout_level`; `risk.ts` Pass 2 force-closes the worst loser at a hard `equity + unrealized < 0` floor; `pricefeed.ts` synthesizes a single global spread (crypto 2 bps, forex/futures 1 bp) with no per-account markup. The doc specifies: (1) new `account_groups` table (`markup_bps`, `leverage`, `stopout_pct`, `is_default`) + `accounts.group_id` (migration **032**, behaviour-preserving default group); (2) integration points — apply markup at order-fill time (NOT in the shared quote cache), group leverage on assignment, group-configurable stop-out in `risk.ts` replacing the 0-floor; (3) admin CRUD + assignment routes + a visual Groups page; (4) a 21.14a–f decomposition table with per-slice acceptance; (5) five open product decisions surfaced for the user (initial group set, markup semantics, stop-out convention, per-account override, credit-bucket interaction), all defaulted to behaviour-preserving choices so 21.14b–e can proceed offline. Resolves the 21.12 dependency. No code/tests changed (markdown only → no deploy). client+server tsc still clean; `npm test` 285 passing.
- **Files:** `docs/account-groups-design.md`
- **Acceptance:** Design doc exists, grounded in the real schema/risk/pricing code, and decomposes 21.14 into offline sub-items with acceptance criteria each. *(Offline — met.)*
### 21.14b Migration `032_account_groups.sql` — table + `accounts.group_id` + default-group seed + backfill *(offline; apply deferred to a network run)*
- [x] **Done 2026-07-09 (auto run 75):** Wrote `supabase/migrations/032_account_groups.sql` per design §2. Creates `account_groups` (`markup_bps`/`leverage`/`stopout_pct`/`is_default` + timestamps), a partial unique index enforcing exactly-one default, seeds the behaviour-preserving `standard` default group (0 markup, 100x, 0% stop-out), adds `accounts.group_id` FK + index, and backfills every existing account into the default group. RLS enabled with no client policies (server service-role only, matching the other server-managed tables). Idempotent guards throughout (IF NOT EXISTS / ON CONFLICT DO NOTHING). client+server `tsc --noEmit` clean; `npm test` 285 passing. **Apply deferred to a network run** (`api.supabase.com` unreachable from the auto-run sandbox): `python scripts/apply-migration.py supabase/migrations/032_account_groups.sql`. **Acceptance met:** migration file present; tsc clean; backfill assigns every existing account to the default group.
### 21.14c Backend group CRUD + `POST /accounts/:id/group` assignment + tests *(offline unit-testable)*
- [ ] Per §3.4. `GET/POST/PATCH/DELETE /api/admin/groups` (admin-only), assignment writes `group_id`+`leverage`, delete reassigns members to default. **Acceptance:** CRUD returns groups with member counts; assignment persists; 403 gating; hermetic tests green.
### 21.14d `applyGroupMarkup` at the order-open fill path + tests *(markup math offline; visual fill deferred)*
- [ ] Per §3.1. Widen buy/sell fills by the group `markup_bps`; house ops (robots/admin) keep the raw mid. **Acceptance:** markup math reconciles in unit tests; shared quote cache untouched.
### 21.14e Group-configurable stop-out in `risk.ts` (resolves 21.12) + tests *(offline unit-testable)*
- [ ] Per §3.3. Replace the `equity + unrealized < 0` floor with a group `stopout_pct` margin-level check; `stopout_pct=0` preserves today's behaviour. **Acceptance:** changing a group's `stopout_pct` changes when the risk worker force-closes a member; tests green.
### 21.14f Admin "Groups" page + account-detail group selector *(visual — screenshot run)*
- [ ] Per §3.4. `app/admin/groups.tsx` + selector on `app/admin/user/`. **Acceptance:** admin can create/edit a group and assign an account; params visibly apply.
...
- **Done 2026-06-19 (auto):** CSV export for all three analytics views via a `?f