...
> **Remaining 18.8 sub-items (not yet carved/done):** 18.8b admin "Robot Runs" screen UI (visual); 18.8c price-alerts log route+screen; 18.8d AI chat-logs log (depends on 18.7 assistant); 18.8e deposits admin approve/reject; 18.8f withdrawals admin approve/reject. Each should be its own bounded item.
...
- [x] **Done 2026-06-20 (auto):** Added 15 new badge codes to `server/src/lib/achievements.ts` (total now 22), each with `ACHIEVEMENT_META` (emoji/label/description) and a `checkX` helper mirroring the existing fire-and-forget pattern: **volume** `volume_1`/`volume_10`/`volume_100` (Σ lots), **trade count** `trades_10`/`trades_50`/`trades_100`, **profit** `first_green`/`profit_100`/`profit_1000` + `gain_10pct` (balance ≥ $11k = +10% on the $10k demo), **discipline** `tp_planner` (10 closed trades w/ a take-profit) + `diversified` (5 distinct symbols), **engagement** `three_day_streak`/`thirty_day_streak`, **robots** `robot_master` (10 built). Wired fire-and-forget at the existing event points: `orders.ts` open (`checkVolumeMilestones`, `checkTradeCountMilestones`, `checkDiversified`) + close (`checkProfitMilestones`, `checkTakeProfitPlanner`, `checkGain10pct`); `robots.ts` create (`checkRobotMaster`); `auth.ts` login streak (3/30). NO migration and NO client change needed: the `achievements` table stores codes as free text, and `app/(tabs)/profile.tsx` renders one tile per `ACHIEVEMENT_META` entry served by `GET /api/achievements`, so the new badges appear automatically. New `server/test/achievements.test.ts` (13 tests: tier thresholds, profit reconciliation incl. open-trade exclusion & loss-only no-op, gain_10pct boundary, tp/diversified/robot counts, no-account no-op, meta completeness=22, idempotency); also extended the achievements `vi.mock` in orders/robots/auth/auth-boundaries/copyTrading/rounds tests with the 7 new fns (routes call them, so the mock must expose them or the open/close routes 500). Verified offline: client tsc clean, server tsc clean, `npm test` **255 passing** (was 242). PENDING LIVE VERIFY (next interactive session): unlock a few new badges (place trades, hit $11k, 3-day streak) and confirm they appear on the profile screen.
